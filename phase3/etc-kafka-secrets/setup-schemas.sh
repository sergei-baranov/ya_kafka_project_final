echo "--- 1. Регистрируем avro-схему из ${PRODUCT_AVRO_SCHEMA_FILE_NAME} для топиков ${TOPIC_GOODS_FILTERED}, ${TOPIC_GOODS_PROHIBITED} ---"
...
echo "--- 2. Регистрируем avro-схему из ${CLIENT_API_SEARCH_AVRO_SCHEMA_FILE_NAME} для топика ${TOPIC_CLIENT_API_SEARCH} ---"

CLIENT_SUBJECT="${TOPIC_CLIENT_API_SEARCH}-value"
CLEAN_SCHEMA_CLIENT=$(
  cat "${CONTAINER_PATH_SECRETS}/${CLIENT_API_SEARCH_AVRO_SCHEMA_FILE_NAME}" \
    | python3 -c "import sys, json; print(json.dumps(sys.stdin.read()))"
)
JSON_BODY_CLIENT="{\"schema\": $CLEAN_SCHEMA_CLIENT}"

echo "--- Работа с $CLIENT_SUBJECT ---"
echo "Установка режима $COMPATIBILITY_LEVEL..."
CONF_RESULT_CLIENT=$(do_schema_registry_rest_curl PUT "/config/$CLIENT_SUBJECT" "$CONFIG_BODY")
echo "Результат: $CONF_RESULT_CLIENT"

CHECK_RESULT_CLIENT=$(do_schema_registry_rest_curl POST "/compatibility/subjects/$CLIENT_SUBJECT/versions/latest" "$JSON_BODY_CLIENT")
echo "CHECK_RESULT (/compatibility/subjects/${CLIENT_SUBJECT}/versions/latest): ${CHECK_RESULT_CLIENT}"

IS_COMPATIBLE_CLIENT=$(echo "$CHECK_RESULT_CLIENT" | python3 -c "import sys, json; data=json.load(sys.stdin); print(data.get('is_compatible', True))")
echo "IS_COMPATIBLE: ${IS_COMPATIBLE_CLIENT}"

if [[ "${IS_COMPATIBLE_CLIENT,,}" == "true" ]]; then
  echo "Схема прошла проверку $COMPATIBILITY_LEVEL."
  REG_RESULT_CLIENT=$(do_schema_registry_rest_curl POST "/subjects/$CLIENT_SUBJECT/versions" "$JSON_BODY_CLIENT")
  NEW_ID_CLIENT=$(echo "$REG_RESULT_CLIENT" | python3 -c "import sys, json; print(json.load(sys.stdin).get('id', 'error'))")
  echo "Зарегистрована! ID: $NEW_ID_CLIENT"
else
  echo "ОШИБКА: Схема client_api_search не соответствует $COMPATIBILITY_LEVEL"
  echo "Детали: $CHECK_RESULT_CLIENT"
  exit 1
fi
