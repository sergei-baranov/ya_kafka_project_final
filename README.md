...
- [Основные Data-pipeline-ы/фазы пайплайнов](#general_descr_data_pipelines)
  ...
  - [Загрузка и фильтрация товаров](#data_pipelines_goods)
    ...
    - фильтрация по стоп-словам в названии
...
Воркер настроен так, что он читает файлы из директории `shop_api_stage` и делает первичный санитайзинг: парсит файлы как `Streaming JSON`, при ошибках перемещает их в директорию `shop_api_error`.

Если же парсинг происходит без ошибок, то каждый json-объект из каждого валидного файла отправляется в Kafka topic `goods-raw`. Успешно обработанные файлы коннектор удаляет (`"cleanup.policy": "DELETE"`).
