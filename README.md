...
- [Инициализационный слой структуры проекта по сервисам (Bootstrap)](#general_descr_init_layer)
  ...
  - [PostgreSQL: настройки производит сам сервис postgres](#init_layer_postgres)
  - [Mirror Maker 1: настройки производит сам сервис mirror-maker](#init_layer_mirror_maker)
  - [Сервис schemas-registrator: загрузка avro-схем в Schema Registry](#init_layer_schemas_registrator)
  - [Сервис connectors-registrator: создание воркера Kafka Connect](#init_layer_connectors_registrator)
  - [Сервис ksqldb-bootstrap: предсоздание стрима и таблицы](#init_layer_ksqldb_bootstrap)
...
- [Основные Data-pipeline-ы/фазы пайплайнов](#general_descr_data_pipelines)
  ...
  - [Загрузка и фильтрация товаров](#data_pipelines_goods)
...
Далее идут разделы, отражающие процесс разработки проекта. Итоговое состояние может сильно отличаться от состояния, описываемого на предыдущих итерациях.
...
## <a name="general_descr_descr">Общее описание</a>

Проект реализует учебно-демонстрационный функционал построения дата-пайплайнов условного маркетплейса: информация по товарам от продавцов, поисковые запросы от покупателей, аналитическая подсистема рекомендаций для покупателей - всё в потоковом режиме обработки (в условном реалтайме).
...
Какие-то из сервисов настраиваются специализированными bootstrap-контейнерами, какие-то сами содержат и запускают свои инициализационные скрипты.

На схеме этот слой структуры проекта по возможности выделен сиреневым цветом.

Важно: все эти процессы требуют времени, зачастую они ресурсоёмки или времяёмки, почти всегда требуют ожидания следующими контейнерами в линейной зависимости, поэтому разворачивание данного compose-проекта происходит "не мгновенно", и это нормально.

### <a name="init_layer_ssl_sasl">SSL(mTLS)/SASL</a>

Практически всем контейнерам подключается volume-ом директория `./etc-kafka-secrets`, сожержащая файлы `kafka.truststore.jks` и `kafka.keystore.pkcs12`.

NB: общий сертификат на все ресурсы - сознательное допущение (упущение), для ускорения разработки в контексте учебного демо-проекта. Включено в Техдолг-раздел.

NB: в целях ускорения процесса разработки учебного проекта данная директория, "раз уж подключается всем", содержит так же другие файлы, необходимые не всем сервисам. Этот момент так же включён в Техдолг-раздел.

Пароли, `pem`, `cert`-файлы и т.п. в данную директорию не помещаются, и при необходимости извлекаются из truststore и keystore в коде контейнеров с использованием паролей, прокидываемых из env-файла.

Сертификаты генерируются руками на хостовой машине до запуска проекта bash-скриптом `make-certs.sh` на основе файлов `ca.cnf`, `kafka.cnf.template` и переменных из `env`-файла.

Если запускаться с файлом `.env.example`, не меняя его и `compose.yaml` в части имён хостов, то генерировать сертификаты не надо, будут работать `kafka.truststore.jks` и `kafka.keystore.pkcs12` из репозитория.

Настройки всех сервисов, которые удалось/успелось, предполагают по возможности `mTLS`, то есть в `SAN`-ы в `kafka.cnf.template` следует включать не только сервера, но и клиентские имена/адреса хостов. Хост `localhost` уже включён, как и ip `127.0.0.1` и `192.168.100.225` (это ip хостовой машины в процессе разработки, оставлен в репозитории для наглядности (чтобы "как бы намекал")).

Что касается `SASL`: если необходимо ввести пользователя в SASL Kafka-кластеров, то надо поправить определённые секции в `compose.yaml` (ищи по коду, например, "entrypoint-with-secrets" или "KafkaServer" и т.д.).

В сервисах у нас прописан `security.protocol/SECURITY_PROTOCOL=SASL_SSL` и `sasl.mechanism/SASL_MECHANISM=PLAIN`, поэтому в инструкции типа `sasl.jaas.config/SASL_JAAS_CONFIG` передаются credential-ы в виде переменных из env-файла, а так же пути к `kafka.truststore.jks` и `kafka.keystore.pkcs12`, так же прописанные в env-файле.

Так как текущий env-файл пробрасывается в контейнеры (кстати, для этого надо в каждом env-файле используемом определить переменную `ENV_PATH`), то те же переменные используются в shell-скриптах контейнеров и т.п.

В данном проекте мы постарались по возможности максимально ограничить права и доступы, поэтому в `KAFKA_SUPER_USERS` прописан минимально необходимый для старта список, и отключены настройки `KAFKA_AUTO_CREATE_TOPICS_ENABLE`, а конкретные ACL-ы конкретным пользователям на конкретные (или по префиксу) топики, группы консьюмеров, кластеры, транзакции и т.д. раздаются bash-скриптами, что описано в след. разделе.

Этот подраздел плавно переходит в следующий, информация трохи размыта по двум разделам доки (потом как-нибудь переформулируем их оба).

### <a name="init_layer_topic_creation">Сервис topic-creation: предсоздание Kafka-топиков, ACL; admin-client.properties</a>

Сервис `topic-creation` подключает обе docker-сети и выполняет предсоздание топиков и создаёт ACL-правила последовательно на обоих Kafka-кластерах.

Топики и правила доступа создаются как необходимые для бизнес-лоики проекта, так и (по возможности), заложенные в сборки используемых инструментов (а это значит, кроме всего прочего, ещё и топики с названиями, зависящими от настроек контейнера, топики с жёстко требуемыми параметрами типа кол-ва партиций, политики удаления и т.п. - всё достаточно хрупко, в том смысле, что, мне кажется, сильно привязано к версиям сборок инструментов и т.п.).

Осуществляет он это при помощи запуска двух скриптов: `setup-acls-stage.sh` и `setup-acls-stage.sh`. Внутри скриптов прописаны разные `--bootstrap-server`, разные наборы топиков и правила ACL, соответствующие бизнес-логике и технической степени защиты проекта. Вся чувствительная информация по возможности прокидывается из env-файла, с которым запускается проект.

При вызове административных инструментов используется аргумент `--command-config` с указанием на файл `admin-client.properties`. Этот файл формируется при разворачивании контейнеров Kafka брокеров и контроллеров с использованием переменных из env-файла. Программный код, формирующий этот файл, прописан в `compose.yaml` в entrypoint-ах брокеров и контроллеров, рядом с созданием `controller.sasl.jaas.conf` и `broker.sasl.jaas.conf`.

**NB**: Расширенные права для `Kafka UI`: очень удобно в процессе отладки использовать веб-интерфейс Kafka UI, но мы вынуждены дать пользоватедю `kafka_ui`, от которого прописали работать сервису, миного прав, иначе уже удобно не будет. Я предполагаю, что на продакшн такие сервисы не едут, или покрываются какими-то жёсткими контурами безопасности, тут я не специалист.

**NB**: Зауженные права у пользователя от `Schema Registry`: для сервиса Schema Registry пришлось не включать настройки идемпотентности и транзакционности, так как не хватило времени на RnD, какие именно префиксы для транзакций и групп надо использовать при настройке правил пользователю `schema_registry_user` (и в ACL ли вообще дело). Этот момент прописан в Техдолг проекта.

### <a name="init_layer_postgres">PostgreSQL: настройки производит сам сервис postgres</a>

На хостовой машине конфигурирующие файлы размещены как `./postgres/init-scripts/create_tables.sql`, `./postgres/custom-config.conf`.

Файл `create_tables.sql` находится в директории, подключаемой volume-ом к контейнеру, маппится в директорию `docker-entrypoint-initdb.d` и его исполнение прописано в `entrypoint`-скрипте используемого образа.

Таким же образом в рсубд в контейнере инъецируется файл `custom-config.conf` (**обратим внимание**: в данном файле содержатся, в частности, настройки потребления ресурсов, которые надо изменять согласованно с изменениями на ограничения по ресурсам контейнера в `compose.yaml`).

В `create_tables.sql` прописано создание таблиц `goods_filtered` и `client_api_search`, необходимых для них триггеров и индексов.

Важно: названия таблиц и т.п. не вынесены в какие-либо настроечные env-файлы или секреты, и в текущей конфигурации для их изменения требуется менять sql в файле `create_tables.sql`.

SQL-выражения из этого файла исполнятся только при первом запуске контейнера (или при запуске после удаления volume-а). При последующих запусках без удаления volume-а этого не происходит, то есть никакие изменения в этих таблицах и пр. не будут reset-иться простым перезапуском контейнера.

Рядом можно добавить несколько других файлов, при этом исполняться все файлы будут в алфавитном порядке. Выражения не ограничены DDL-ом, таким образом можно не только создать структуры данные или серверные рутины, но и заполнить инициализационные данные для проекта.

### <a name="init_layer_mirror_maker">Mirror Maker 1: настройки производит сам сервис mirror-maker</a>

В `compose.yaml` в `command` данного сервиса прописана "простыня" bash-инструкций, которые сначала создают конфигурационные файлы `consumer.properties` и `producer.properties`, а затем запускают процесс `kafka-mirror-maker`, передавая ему, в частности, аргументы с этими файлами.

Сервис таким образом настраивается на репликацию двух топиков из stage-кластера Kafka в mart-кластер.

В качестве сериализатора/десериализатора как ключа, так и значения указываются `ByteArraySerializer`/`ByteArrayDeserializer`, поэтому связка со схемами не требуется, и этот сервис получается одним из самых простых (но он Deprecated поставщиком, о чём и сообщает в логах контейнера).


### <a name="init_layer_schemas_registrator">Сервис schemas-registrator: загрузка avro-схем в Schema Registry</a>

У нас в системе 4 топика связаны с avro-схемами.

Это топики `goods-filtered`, `goods-prohibited`, `client-api-search` и `client-recommendations`.

Топики `goods-filtered` и `goods-prohibited` не имеют ключа, и им регистрируется схема только на `goods-filtered-value` и `goods-prohibited-value`.
Сообщения обоих этих топиков должны соответствовать в части value одинаковой схеме, которая у нас хранится в файле `./etc-kafka-secrets/product.avsc`.

Топик `client-api-search` так же не имеет ключа в сообщениях, для него регистрируется схема `client-api-search-value`, которая у нас находится в файле `./etc-kafka-secrets/client_api_search.avsc`

Топик `client-recommendations` имеет схемы и для значения, и для ключа. Схемы регистрируем как `client-recommendations-value` и `client-recommendations-key`, и находятся они у нас в файлах `./etc-kafka-secrets/client_recommendations_value.avsc` и `./etc-kafka-secrets/client_recommendations_key.avsc` соответственно.

Режим совместимости всем схемам задаётся `FULL`.

Регистрациюя всех этих схем в сервисе schema-registry прописана в скрипте `./etc-kafka-secrets/setup-schemas.sh`, а на исполнение его отправляет, предварительно дождавшись доступности REST-интерфейса контейнера со `Schema Registry`, как раз контейнер `schemas-registrator`.

Далее эти схемы, последних версий , запрашиваются у `Schema Registry` всеми работающими с данными топиками инструментами.

### <a name="init_layer_connectors_registrator">Сервис connectors-registrator: создание воркера Kafka Connect</a>

У нас в системе используется всего один, но важный воркер-коннектор для Kafka Connect.

Его определение находится у нас в файле `./etc-kafka-secrets/kafka-connect_shop_api.conf.json`.

Воркер (конфиг) регистрируется под именем `shop-api-stage-reader` (это прописано в json-схеме).

Регистрирует его специальный сервис `connectors-registrator`: он дожидается доступности REST-нтерфейса `Kafka Connect` в контейнере `kafka-connect` и POST-ит ему схему из файла.

Данный коннектор читает `streaming json` файлы из директории `shop_api_stage` и отправляет сообщения в топик `goods-raw` stage-кластера, используя класс `SpoolDirSchemaLessJsonSourceConnector`.

Топик не привязан к какой-либо схеме, валидация товаров по схеме происходит позже, перед фильтрацией.

Если же файл или товар в нём не распознаётся как json вовсе, то файл перемещается в директорию `shop_api_error` (при успехе - удаляется (это прописывается в конфиге коннектора)).

### <a name="init_layer_ksqldb_bootstrap">Сервис ksqldb-bootstrap: предсоздание стрима и таблицы</a>

У нас используется ksqlDB для получения списка рекомендаций.

Список рекомендаций формируется в потоке, отправляется в Kafka-топик, рекомендации могут постоянно меняться в зависимости от поисковой активности пользователя, а сервис получения хочет только посдледнюю (самую актуальную) из них. Для этого мы применяем, в частности, ksqlDB с CAS-таблицей.

Сервис `ksqldb-bootstrap` определяет поток и таблицу в сервисе `ksqldb-server`.

Делает он это, запуская скрипт `ksqldb-bootstrap.sh`, после яего прекращает работу.

Скрипт `ksqldb-bootstrap.sh` определяет стрим client_recommendations_s, привязанный к топику `client-recommendations` и CAS-таблицу `client_recommendations_latest` от этого стрима.

Из интересного: сообщения в топике завязаны на avro-схему и на value, и на key, и у нас везде по этому поводу... интересное :).

### <a name="init_layer_spark">Настройки Spark-нод</a>

В директории `./spark` находится `Dockerfile`, его `entrypoint.sh` и Spark job `recommendations_job.py`.

Всё, что необходимо для конфигурирования работы сервиса `spark-recommendations-job`, прописано там.

Сервисы `spark-master` и `spark-worker` ни в какой особой конфигшурации не нуждаются сверх переменных сервисам в `compose.yaml` (**но сама конфигурация важна**, обратим внимание например на настройки потребления памяти для сервиса `spark-worker`, внешние и внутренние для контейнера).

### <a name="init_layer_shop_api_app">Настройка shop-api-app</a>

Сервис `shop-api-app` собирается из Dockerfile в директории `./shop-api-app`, все настройки и пр. - там.
...
Техдолг: Намутить так, чтобы при передаче False было удвление записи из таблицы, из топика... Ну то есть не удаление, а пометка к удалению `null` (`tombstone`). Посмотреть, что с этим у Фауста, если ничего - своим кодом отмечать, настроив, кстати, топик соотв. образом.

### <a name="data_pipelines_goods">Загрузка и фильтрация товаров</a>

#### <a name="data_pipelines_goods_trigger_ingest">Phase 1: Trigger Event, Ingestion & Sanitization Layer (Kafka Connect)</a>

Инициализирующим событием (Trigger Event) для данного дата-пайплайна является появление JSON-файла в директории сканирования коннектора `SpoolDirSchemaLessJsonSourceConnector`.

Таковой директорией для стейджинга файлов мы указали директорию `shop_api_stage`: с хостовой машины она доступна как `./kafka-connect/data/shop_api_stage`, сервису `kafka-connect` как `/data/shop_api_stage`, за счёт такого определения volume-ов для контейнера:

```
- ./kafka-connect/data/shop_api_stage:/data/shop_api_stage
- ./kafka-connect/data/shop_api_error:/data/shop_api_error
```

На данной фазе работает сервис `kafka-connect`: на bootstrap-фазе развёртывания мы зарегистрировали в нём воркер класса `SpoolDirSchemaLessJsonSourceConnector` (см. выше [Сервис connectors-registrator: создание воркера Kafka Connect](#init_layer_connectors_registrator)), переда в Kafka Connect конфигурацию из файла `./etc-kafka-secrets/kafka-connect_shop_api.conf.json`.

Воркер настроен так, что он читает файлы из директории `shop_api_stage` и делает первичный санитайзинг: парсит файлы как `Streaming JSON` (pretty-форматированные json-объекты, разделённые пустыми строками), при ошибках перемещает их в директорию `shop_api_error`.

Если же парсинг происходит без ошибок, то каждый json-объект из каждого валидного файла отправляется в Kafka topic `goods-raw` `stage-кластера` Кафки. Успешно обработанные файлы коннектор удаляет (`"cleanup.policy": "DELETE"`).

Топику `goods-raw` не сопоставлена никакая схема, сообщения туда едут просто как валидный json, и ждут дальнейшей обработки.

Для тестирования проекта в репозитории есть фикстуры: деректория `./shop_api_fixtures`. Для эмуляции потока от магазинов просто копируем файлы из неё (все вместе или по очереди с задержками) в директорию, которую сканирует Kafka Connector согласно нашим настройкам.

#### <a name="data_pipelines_goods_dlq">Phase 2: Schema Validation & DLQ Routing (Faust-streaming agent, Schema Registry)</a>

На данной фазе сообщения из топика `goods-raw` `stage-кластера` Кафки проверяются на соответствие avro-схеме `goods-filtered-value` в Schema Registry, и не соответствующие схеме копируются в schemaless-топик `goods-dlq` того же кластера, после чего исключаются из потока обработки.

Воркером данной фазы выступает Faust agent `validator_agent` (see `./shop-api-app/app/shop_api/agents.py`) сервиса `shop-api-app`.

Для проверки соответствия сообщения avro-схеме воркер получает схему из Schema Registry, а в нём схемы все предварительно уже зарегистрированы на Bootstrap-фазе развёртывания проекта (см. [Сервис schemas-registrator: загрузка avro-схем в Schema Registry](#init_layer_schemas_registrator)).

Таким образом на данной фазе обеспечивается Contract Enforcement для последующих фаз дата-пайплайна.

#### <a name="data_pipelines_goods_filtering">Phase 3: Content Filtering & Moderation + репликация на mart-кластер</a>

Эту фазу выделяем логически, а технически она реализуется в том же Faust агенте `validator_agent`: после проверки на соответствие схеме мы ищем в названии товара (поле `name` согласно схеме) вхождения построк из списка, хранящегося в топике `prohibition-list`.

Это список управляется администраторами через CLI API (см. [Управление списком запрещённых в названиях товаров слов](#data_pipelines_prohibition_list)).

Товары (записи), не прошедшие фильтр по стоп-словам, отправляются в топик `goods-prohibited` `stage`-кластера Кафки.

Топику `goods-prohibited` так же сопоставлена avro-схема (`goods-prohobited-value`) в Schema Registry, поэтому перед отправкой сообщения в топик проверяется его соответствииие схеме. При несоответствии схеме, сообщение так же едет в schemaless-топик `goods-dlq`.

*(таким образом, в идеале avro-схемы `goods-prohobited-value` и `goods-filtered-value` должны быть идентичны или совместимы (этот вопрос я не исследовал, просто создал две идентичные схемы, запиcал в Техдолг-лист проекта))*

Оставщиеся сообщения, то есть прошедшие валидацию по схемам и модерацию по стоп-словам, едут в топик `goods-filtered` `stage`-кластера.

**Тут же отметим:** Этот топик реплицируется на `mart`-кластер Кафки сервисом `mirror-maker`, что настраивается на Bootstrap-фазе развёртывания проекта (см. [Mirror Maker 1: настройки производит сам сервис mirror-maker](#init_layer_mirror_maker))
...
Технически эта фаза реализуется в том же Faust агенте `validator_agent`, асинхронно отправке отфильтрованных сообщений в Кафку. См. `GoodsFilteredBatchSink`, `enqueue_filtered_product`, etc.

Таблица, её триггеры, индексы и т.п. предварительно определены в рсубд на Bootstrap-фазе развёртывания проекта (см. [PostgreSQL: настройки производит сам сервис postgres](#init_layer_postgres)).

За счёт дедупликации в микробатчах, отправляемых в рсубд, а так же sql-запроса с условным upsert, таблица `goods_filtered` в PostgreSQL персистирует в почти-реалтайме снимок самых свежих состояний товаров (ориентируемся на обязательное поле `updated_at` avro-схемы товара; Техдолг: поле текстовое, отсюда определённые танцы, и надо изучить этот вопрос поглубже).

Далее эта таблица используется в коротком пайплайне "[Поиск товаров по названию](#data_pipelines_search_goods)"
...
Поле `product_data` - `JSONB`-поле, с триграммным `GIN`-индексом на `product_data ->> 'name'`.
...
Avro-схемы в Schema Registry, настройка Mirror Maker, топики Кафки, таблица Постгреса, доступы и т.п. - всё подготавливается на Bootstrap-фазе развёртывания проекта и тоже описано выше.
...
Всё было бы просто, если бы не avro... Но не стал упрощаться и убирать схему, чтобы не забыть на досуге покопать в это всё ("не самая красивая" работа со стримовыми датафреймами во Spark-е при наличии avro-схем у сообщений топика).

Топик `client-recommendations` используется как первичный источник данных Faust-операции `/get-recommendations` (через обработку в `ksqlDB`).
...
Чтение avro-схем ksqlDB делает сам, настройки для связи со Schema Registry прописаны у сервиса `ksqldb-server` в `compose.yaml`. См. тж. DDL в `ksqldb-bootstrap.sh`.
...
- совместимость схем `goods-prohobited-value` и `goods-filtered-value` в рантайме (возмождности изменения одной из схем: изучить,ю расписать; тут же: возможность привязки нескольких топиков к одной схеме в реестре, чтобы все воркеры разных подсистем это понимали как-то "на автомате" (сейчас все ориентируются на название топика с суффиксом `-key` и `-value`)); вообще изучай детальнее работу со схемами во всех сервисах, которые на неё завязаны при работе с топиками.
- Не все bootstrap-сервисы дожидаются доступности таргетных сервисов
- у Kafka UI не появляется в выпадающих списках вариант применить avro-схемы из Schema Registry для топика, когда мы идём в закладку "Сообщения". Возможно я что-то недонастроил.
- ...

## <a name="general_assignment_review">Как проверять проект</a>

- Чтобы просто проверить исполнение - смотрим код в файлах и исполняемся по инструкциям в разделе ["Быстрая проверка"](#fast_assignment_review),
- Чтобы проверить ход выполнения проекта - читаем следующие за ним разделы.


## <a name="fast_assignment_review">Быстрая проверка</a>

NB: везде, где далее по тексту встречается "192.168.100.225", у вас при развёртывании будет "localhost" или адрес вашей хостовой машины. Если необходимо - надо внести этот адрес в SAN-ы шаблона сертификата и перегенерировать сертификаты скриптом `make-certs.sh` (ниже по тексту, если дочитаете, мы это делаем на какой-то из первых итераций).
...
NB: конрейнерам ограничены ресурсы в `compose.yaml` (значения заданы переменными в `env`-файле) под работоспособность на хостовой машине с ограниченными ресурсами (32G RAM). При необходимости можно увеличить значения в env-файле (тут аккуратно: у некоторых сервисов внутри контейнеров есть настройки в файлах конфигураций, которые надо соотносить со внешними ограничениями контейнеров).

NB: рекомендую разворачивать проект с аргументом `--env-file .env.example` или переименовать/скопировать `.emv.example` в `.env` (и **поменять значение переменной** `ENV_PATH` в этом файле (!!!)). Но лучше тестировать начинать на `--env-file .env.example` всё же (мало ли в каких bash-скриптах я его использовал с форсированным в коде путём).

Не рекомендую менять сразу значения переменных в env-файле, так как не всё покрывается переменными, и для многих настроек сервисов меняя переменные в env-файле надо согласованно поменять строки в файлах конфигураций, и даже в compose.yaml (ключи словарей).

Для быстрой проверки мы
- развернём compose-проект,
- зададим руками (через cli api) стоп-слова для фильтрации запрещённых товаров по названиям,
- зальём фикстуры потока товаров от магазинов в директорию, которую прочитает Kafka Connect,
- и далее отработает пайплайн,
- а мы сделаем пару поисков товаров от разных пользователей по http api,
- и так же по http api посмотрим на условные рекомендации от аналитической подсистемы.
...
**NB**: у Kafka UI не появляется в выпадающих списках вариант применить avro-схемы из Schema Registry для топика, когда мы идём в закладку "Сообщения". Возможно я что-то недонастроил.
...
Ну можно посмотреть логи Spark job-а:
...
**Техдолг: в нашем славном mTLS-царстве затесался ренегат.** В будущем надо закрыть `kafka-connect` на SSL.
...
**Всё прекрасно: поисковые запросы пользователей в почти-реалтайме превращаются в квази-рекомендации в топике client-recommendations mart-кластера Кафки при помощи pyaspark-job-а.**
...
- добить README, в т.ч. нарисовать сему сервисов по факту, адреса ui для sopark-а кажется забыл указать, и т.п.
