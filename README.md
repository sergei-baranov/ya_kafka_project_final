...
Данный коннектор читает `streaming json` файлы из директории `shop_api_stage` и отправляет сообщения в топик `goods-raw` mart-кластера, используя класс `SpoolDirSchemaLessJsonSourceConnector`.
