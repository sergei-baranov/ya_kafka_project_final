...
NB: общий сертификат на все ресурсы - созгательное допущение (упущение), для ускорения разработки в контексте учебного демо-проекта. Включено в TODO-раздел.

NB: в целях ускорения процесса разработки учебного проекта данная директория, "раз уж подключается всем", содержит так же другие файлы, необходимые не всем сервисам. Этот момент так же включёно в TODO-раздел.
...
Этот подраздел плавно переходит в следующий, информация трохи размыта по двум разделам доки (todo).
...
За счёт дедупликации в микробатчах, отправляемых в рсубд, а так же sql-запроса с условным usert, таблица `goods_filtered` в PostgreSQL персистирует в почти-реалтайме снимок самых свежих состояний товаров (ориентируемся на обязательное поле `updated_at` avro-схемы товара; todo: поле текстовое, отсюда определённые танцы, и надо изучить этот вопрос поглубже).
