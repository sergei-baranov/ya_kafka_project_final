...
Какие-то из сервисов настраиваются специализированными контейнерами, какие-то сами содержат и запускают свои инициализационные скрипты.
...
NB: общий сертификат на все ресурсы - созгательное допущение (упущение), для ускорения разработки в контексте учебного демо-проекта. Включено в Техдолг-раздел.

NB: в целях ускорения процесса разработки учебного проекта данная директория, "раз уж подключается всем", содержит так же другие файлы, необходимые не всем сервисам. Этот момент так же включёно в Техдолг-раздел.
...
В данном проекте мы постарались по возможности максимально ограничить права и доступы, поэтому в `KAFKA_SUPER_USERS` прописан минимально необходимый для старта список, и отключены настройки `KAFKA_AUTO_CREATE_TOPICS_ENABLE`, а конкретные ACL-ы конкретным пользователям на конкретные (или по префиксу) топики, группы консьюмеров, кластера, транзакции и т.д. раздаются bash-скриптами, о чём описано в след. разделе.
...
В качестве сериализатора/десериализатора как ключа, так и значения указываются `ByteArraySerializer`/`ByteArrayDeserializer`, поэтому связка со схемами не требуется, и этот сервис получается одним из самых простых (даром что Deprecated).
...
У нас в системе используется всего один, но ультимативно важный воркер-коннектор для Kafka Connect.

Его дефиниция находится у нас в файле `./etc-kafka-secrets/kafka-connect_shop_api.conf.json`.
...
Техдолг: Намутить так, чтобы при передачи False было удвление записи из таблицы, из топика... Ну то есть не удаление, а пометка к удалению `null` (`tombstone`). Посмотреть, что с этим у Фауста, если ничего - своим кодом отмечать, настроив, кстати, топик соотв. образом.
...
Таким образом на данной фазе обеспечмвается Contract Enforcement для последующих фаз дата-пайплайна.

#### <a name="data_pipelines_goods_filtering">Phase 3: Content Filtering & Moderation + репликация на mart-кластер</a>

Эту фазу выделяю логически, но технически она реализуется в том же Faust агенте `validator_agent`: после проверки на соответствие схеме мы ищем в названии товара (поле `name` согласно схеме) вхождения построк из списка, хранящегося в топике `prohibition-list`.
...
*(таким образом, в идеале avro-схемы `goods-prohobited-value` и `goods-filtered-value` доддны быть идентичны или совместимы (этот вопрос я не исследовал, просто создал две идентичные схемы, запиcал в ТУДУ-лист проекта))*
...
За счёт дедупликации в микробатчах, отправляемых в рсубд, а так же sql-запроса с условным usert, таблица `goods_filtered` в PostgreSQL персистирует в почти-реалтайме снимок самых свежих состояний товаров (ориентируемся на обязательное поле `updated_at` avro-схемы товара; Техдолг: поле текстовое, отсюда определённые танцы, и надо изучить этот вопрос поглубже).
...
NB: рекомендую разворачивать проект с аргументом `--env-file .env.example` или переименовать/скопировать `.emv.example` в `.env` (и поменять значение переменной `ENV_PATH` в этом файле (!!!)). Но лучше тестировать начинать на `--env-file .env.example` всё же (мало ли в каких bash-скриптах я его использовал и забыл заменить на переменную...).

Не рекомендую менять сразу значения переменных в env-файле, так как не всё покрыто переменными, и для многих настроек сервисов меняя переменные в env-файле надо согласованно поменять строки в файлах конфигураций, и даже в compose.yaml (ключи словарей).
